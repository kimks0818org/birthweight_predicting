...
@author: Pongtit, Mai, Michelle, Leina, Yunhyung, Kisuc
...
# possible outliers, which we might flag.
...
'''
By looking at correlation analysis, we found that some variables should be
...
# Creating target dataset by selecting birth weight which is our
# predicted variable
...
# Step 1: Build the model with all variables to see the model
...
# Step 3: Analyze the summary output. After putting all variables, the
# result is not good enough so we have to remove some variables to create
# a good model for prediction.
...
print("""
      ...
      is 0.728 and testing score is 0.709. The results of training and
      testing are not much different, which means our prediction model is not
      overfitting.
      """)
...
# enough since the number of neighbor is not the best suited number in this
# model.
print(y_score)
...
print(f"""
      ...
      from KNN models, which still is not good enough when we compare to OLS 
      model.
      """)
...
# root mean squared error to compare how far we are compared to actual data.
...
print(f"""
      ...
      actual data compared to KNN model. OLS model predicts approximately 
      {lr_rmse.round(0)} grams away from the data while KNN predicts  
      approximately {knn_rmse.round(0)} grams away.
      """)
...
print("""
      In conclusion, from this case, OLS model could predict more accurately
      compared to KNN model.
      """)
...
level of education might be correlated with lower ability and access to 
care for the baby while pregnant thus we decided to create lower flags for
outliers of education to be less than high school level of education.

For father's age, although the men do not have menopause to worry about and
...
might positively and negatively affect birth weight.

For month prenatal care began, earlier access to care can lead to more
education and information about the process. In addition, it can mean an 
earlier start to supplements and caring for the growing child which in 
turn can impact birth weight.
